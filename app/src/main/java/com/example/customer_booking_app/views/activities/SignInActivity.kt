...
import com.example.customer_booking_app.models.LoginAccount
...
class SignInActivity: AppCompatActivity()  {
    ...
    private fun callSignInApi(){
        ...
        GlobalScope.launch(Dispatchers.IO){
            val resLogIn = api.loginCustomer(acc).awaitResponse()
            ...
            if (resLogIn.code() == 401){
                ...
                val resGetProfile = api.getProfileCustomer(Information.token, bodyData).awaitResponse()
                if(resGetProfile.code() == 200) {
                    Information.email = resGetProfile.body()!!.passengers[0].email
                    Information.phone = resGetProfile.body()!!.passengers[0].phone
                    withContext(Dispatchers.Main){
                        progressDialog.hide()
